...
- [x] #2 Écrire le `docker-compose.yml` (services : php, nginx/caddy, mailhog) — via `compose.yaml`/`compose.override.yaml` (convention Symfony Flex), service mail = Mailpit ; pas de service BDD (voir #3)
- [x] #3 Configurer la connexion Doctrine (`.env`, `doctrine.yaml`) — **SQLite** (fichier local dans `var/`) au lieu de PostgreSQL : projet 100% local, pas de service BDD à faire tourner
